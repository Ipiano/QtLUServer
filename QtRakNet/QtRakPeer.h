...
#include "./RakNet3.25/MessageIdentifiers.h"
#include "./RakNet3.25/RakPeerInterface.h"

class QtRakPeer : public QObject
{
    Q_OBJECT

    mutable QMutex _peerMutex;
    QMutex _runMutex;
    QFutureWatcher<void> _thread;
...
signals:
    void Recieve(unsigned int, SystemIndex, SystemAddress, QByteArray);

public:
    QtRakPeer(unsigned int sleepInterval = 30, QObject* parent=NULL);
    ...
    virtual bool Startup( unsigned short maxConnections, int _threadSleepTimer, SocketDescriptor *socketDescriptors, unsigned socketDescriptorCount )
    {
        QMutexLocker lock(&_peerMutex);
        if(running()) return false;

        int success = _peer->Startup(maxConnections, _threadSleepTimer, socketDescriptors, socketDescriptorCount);
        if(success)
        {
            running(true);
            _thread.setFuture(QtConcurrent::run(this, &QtRakPeer::pollRakPeer));
        }
    ...
    virtual void InitializeSecurity(const char *pubKeyE, const char *pubKeyN, const char *privKeyP, const char *privKeyQ )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->InitializeSecurity(pubKeyE, pubKeyN, privKeyP, privKeyQ);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->AddToSecurityExceptionList(ip);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->RemoveFromSecurityExceptionList(ip);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->IsInSecurityExceptionList(ip);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetMaximumIncomingConnections(numberAllowed);
    }
    ...
    virtual unsigned short GetMaximumIncomingConnections( void ) const
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetMaximumIncomingConnections();
    }

    /// Returns how many open connections there are at this time
    /// \return the number of open connections
    virtual unsigned short NumberOfConnections(void) const
    {
        QMutexLocker lock(&_peerMutex);
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetIncomingPassword(passwordData, passwordDataLength);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->GetIncomingPassword(passwordData, passwordDataLength);
    }
    ...
    virtual bool Connect( const char* host, unsigned short remotePort, const char *passwordData, int passwordDataLength, unsigned connectionSocketIndex=0 )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->Connect(host, remotePort, passwordData, passwordDataLength, connectionSocketIndex);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->Shutdown(blockDuration, orderingChannel);
        running(false);
    }
    ...
    virtual bool GetConnectionList( SystemAddress *remoteSystems, unsigned short *numberOfSystems ) const
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetConnectionList(remoteSystems, numberOfSystems);
    }
    ...
    virtual bool Send( const char *data, const int length, PacketPriority priority, PacketReliability reliability, char orderingChannel, SystemAddress systemAddress, bool broadcast )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->Send(data, length, priority, reliability, orderingChannel, systemAddress, broadcast);
    }
    ...
    virtual bool Send( const RakNet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, SystemAddress systemAddress, bool broadcast )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->Send(bitStream, priority, reliability, orderingChannel, systemAddress, broadcast);
    }

    virtual bool Send( const QByteArray& message, PacketPriority priority, PacketReliability reliability, char orderingChannel, SystemAddress systemAddress, bool broadcast )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->Send(message.constData(), message.size()+1, priority, reliability, orderingChannel, systemAddress, broadcast);
    }
    ...
    /// \param[in] broadcast True to send this packet to all connected systems. If true, then systemAddress specifies who not to send the packet to.
    /// \return False if we are not connected to the specified recipient.  True otherwise
    virtual bool SendList( char **data, const int *lengths, const int numParameters, PacketPriority priority, PacketReliability reliability, char orderingChannel, SystemAddress systemAddress, bool broadcast )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->SendList(data, lengths, numParameters, priority, reliability, orderingChannel, systemAddress, broadcast);
    }

    /// Return the total number of connections we are allowed
    // TODO - rename for RakNet 3.0
    virtual unsigned short GetMaximumNumberOfPeers( void ) const
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetMaximumNumberOfPeers();
    }
    ...
    virtual void RegisterAsRemoteProcedureCall( const char* uniqueID, void ( *functionPointer ) ( RPCParameters *rpcParms ) )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->RegisterAsRemoteProcedureCall(uniqueID, functionPointer);
    }
    ...
    virtual void RegisterClassMemberRPC( const char* uniqueID, void *functionPointer )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->RegisterClassMemberRPC(uniqueID, functionPointer);
    }
    ...
    virtual void UnregisterAsRemoteProcedureCall( const char* uniqueID )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->UnregisterAsRemoteProcedureCall(uniqueID);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetNetworkIDManager(manager);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetNetworkIDManager();
    }
    ...
    virtual bool RPC( const char* uniqueID, const char *data, BitSize_t bitLength, PacketPriority priority, PacketReliability reliability, char orderingChannel, SystemAddress systemAddress, bool broadcast, RakNetTime *includedTimestamp, NetworkID networkID, RakNet::BitStream *replyFromTarget )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->RPC(uniqueID, data, bitLength, priority, reliability, orderingChannel, systemAddress, broadcast, includedTimestamp, networkID, replyFromTarget);
    }
    ...
    /// \param[in] replyFromTarget If 0, this function is non-blocking.  Otherwise it will block while waiting for a reply from the target procedure, which should be remotely written to RPCParameters::replyToSender and copied to replyFromTarget.  The block will return early on disconnect or if the sent packet is unreliable and more than 3X the ping has elapsed.
    /// \return True on a successful packet send (this does not indicate the recipient performed the call), false on failure
    virtual bool RPC( const char* uniqueID, const RakNet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, SystemAddress systemAddress, bool broadcast, RakNetTime *includedTimestamp, NetworkID networkID, RakNet::BitStream *replyFromTarget )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->RPC(uniqueID, bitStream, priority, reliability, orderingChannel, systemAddress, broadcast, includedTimestamp, networkID, replyFromTarget);
    }
    ...
    virtual void CloseConnection( const SystemAddress target, bool sendDisconnectionNotification, unsigned char orderingChannel=0 )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->CloseConnection(target, sendDisconnectionNotification, orderingChannel);
    }
    ...
    virtual bool IsConnected(const SystemAddress systemAddress, bool includeInProgress=false, bool includeDisconnecting=false)
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->IsConnected(systemAddress, includeInProgress, includeDisconnecting);
    }
    ...
    virtual int GetIndexFromSystemAddress( const SystemAddress systemAddress )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetIndexFromSystemAddress(systemAddress);
    }
    ...
    virtual SystemAddress GetSystemAddressFromIndex( int index )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetSystemAddressFromIndex(index);
    }
    ...
    virtual void AddToBanList( const char *IP, RakNetTime milliseconds=0 )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->AddToBanList(IP, milliseconds);
    }
    ...
    virtual void Ping( const SystemAddress target )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->Ping(target);
    }
    ...
    /// \param[in] connectionSocketIndex Index into the array of socket descriptors passed to socketDescriptors in RakPeer::Startup() to send on.
    virtual void Ping( const char* host, unsigned short remotePort, bool onlyReplyOnAcceptingConnections, unsigned connectionSocketIndex=0 )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->Ping(host, remotePort, onlyReplyOnAcceptingConnections, connectionSocketIndex);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetAveragePing(systemAddress);
    }
    ...
    virtual int GetLastPing( const SystemAddress systemAddress ) const
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetLastPing(systemAddress);
    }

    /// Returns the lowest ping time read or -1 if none read yet
    /// \param[in] systemAddress Which system we are referring to
    /// \return The lowest ping time for this system, or -1
    virtual int GetLowestPing( const SystemAddress systemAddress ) const
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetLowestPing(systemAddress);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetOccasionalPing(doPing);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetOfflinePingResponse(data, length);
    }
    ...
    virtual void GetOfflinePingResponse( char **data, unsigned int *length )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->GetOfflinePingResponse(data, length);
    }
    ...
    virtual SystemAddress GetInternalID( const SystemAddress systemAddress=UNASSIGNED_SYSTEM_ADDRESS ) const
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetInternalID(systemAddress);
    }
    ...
    virtual SystemAddress GetExternalID( const SystemAddress target ) const
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetExternalID(target);
    }
    ...
    virtual void SetTimeoutTime( RakNetTime timeMS, const SystemAddress target )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetTimeoutTime(timeMS, target);
    }
    ...
    virtual int GetMTUSize( const SystemAddress target ) const
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetMTUSize(target);
    }

    /// Returns the number of IP addresses this system has internally. Get the actual addresses from GetLocalIP()
    virtual unsigned GetNumberOfAddresses( void )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetNumberOfAddresses();
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->AllowConnectionResponseIPMigration(allow);
    }
    ...
    virtual bool AdvertiseSystem( const char *host, unsigned short remotePort, const char *data, int dataLength, unsigned connectionSocketIndex=0 )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->AdvertiseSystem(host, remotePort, data, dataLength, connectionSocketIndex);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetSplitMessageProgressInterval(interval);
    }
    ...
    virtual void SetUnreliableTimeout(RakNetTime timeoutMS)
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetUnreliableTimeout(timeoutMS);
    }
    ...
    virtual void SendTTL( const char* host, unsigned short remotePort, int ttl, unsigned connectionSocketIndex=0 )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SendTTL(host, remotePort, ttl, connectionSocketIndex);
    }
    ...
    virtual void SetCompileFrequencyTable( bool doCompile )
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetCompileFrequencyTable(doCompile);
    }
    ...
    virtual bool GetOutgoingFrequencyTable( unsigned int outputFrequencyTable[ 256 ] )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetOutgoingFrequencyTable(outputFrequencyTable);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GenerateCompressionLayer(inputFrequencyTable, inputLayer);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->DeleteCompressionLayer(inputLayer);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetCompressionRatio();
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetDecompressionRatio();
    }
    ...
    virtual void AttachPlugin( PluginInterface *plugin )
    {
        QMutexLocker lock(&_peerMutex);
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->DetachPlugin(messageHandler);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->PushBackPacket(packet, pushAtHead);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetRouterInterface(routerInterface);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->RemoveRouterInterface(routerInterface);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->AllocatePacket(dataSize);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->ApplyNetworkSimulator(maxSendBPS, minExtraPing, extraPingVariance);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        _peer->SetPerConnectionOutgoingBandwidthLimit(maxBitsPerSecond);
    }
    ...
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->IsNetworkSimulatorActive();
    }
    ...
    virtual RakNetStatistics * const GetStatistics( const SystemAddress systemAddress )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetStatistics(systemAddress);
    }

    // --------------------------------------------------------------------------------------------EVERYTHING AFTER THIS COMMENT IS FOR INTERNAL USE ONLY--------------------------------------------------------------------------------------------
    /// \internal
    virtual char *GetRPCString( const char *data, const BitSize_t bitSize, const SystemAddress systemAddress)
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->GetRPCString(data, bitSize, systemAddress);
    }

    /// \internal
    virtual bool SendOutOfBand(const char *host, unsigned short remotePort, MessageID header, const char *data, BitSize_t dataLength, unsigned connectionSocketIndex=0 )
    {
        QMutexLocker lock(&_peerMutex);
        return _peer->SendOutOfBand(host, remotePort, header, data, dataLength, connectionSocketIndex);
    }

private:
    unsigned char GetPacketIdentifier(const QByteArray& data)
    {
        if ((unsigned char)data[0] == ID_TIMESTAMP)
        {
            if(data.size() <= sizeof(unsigned char) + sizeof(unsigned long)) return ID_UNKNOWN;
            int out = sizeof(unsigned char) + sizeof(unsigned long);
            return data[out];
        }
        else
            return data[0];
